...
class AuthService {
  void signupUser({
    ...
    try {
      ...
      http.Response res = await http.post(
        Uri.parse('$uri/users/create-account'),
        body: user.toJson(),
    ...
    try {
      ...
      httpErrorHandle(
        ...
        onSuccess: () async {
          ...
          await prefs.setString('ax-auth-token', jsonDecode(res.body)['token']);
  ...
  void logOut(BuildContext context) async {
    try {
      ...
      await sharedPreferences.setString("ax-auth-token", '');
      Navigator.pushNamedAndRemoveUntil(
        context,
  ...
  void getUserData({
    ...
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString('ax-auth-token');

      if (token == null) {
        prefs.setString('ax-auth-token', '');
      }

      var tokenRes = await http.post(
        Uri.parse('$uri/users/is-token-valid'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'ax-auth-token': token!
        },
